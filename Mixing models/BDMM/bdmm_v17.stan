...
//attention! here t_S1 in lambda is because t_S1=t_obs in Marin et al.
  ...
  vector bdmm(real time, vector y, 
              ...
              vector lambda, int K, matrix s1, vector t_S1, matrix s2, vector t_S2, 
    ...
    derivative[1] = lambda_over_time(t_S1,lambda,time) * (mix1 - y[1]);
    derivative[2] = lambda_over_time(t_S2,lambda,time) * (mix2 - y[2]);
...
generated quantities {
  ...
  y_fit = ode_bdf_tol(bdmm, y0, t0, to_array_1d(t_fit), 
                       rel_tol, abs_tol, max_num_steps, 
                       lambda_mean, K, s1, t_obs, s2, t_obs, q1, q2, D, beta, t_mean, t_sd);
