functions {
  ...
  real Mix(int K, real time, vector S, vector q, int D, matrix beta, real t_mean, real t_sd) {
    vector[K] p = P_poly(time, K, D, beta, t_mean, t_sd);
    vector[K] numerator = p .* q .* S;
  ...
  vector bdmm(real time,
    ...
    vector[2] derivative;
    derivative[1] = lambda * (-y[1] + Mix(K, time, s1, q1, D, beta, t_mean, t_sd));
    derivative[2] = lambda * (-y[2] + Mix(K, time, s2, q2, D, beta, t_mean, t_sd));
    return derivative;
  }
}
...
data {
  ...
  array[n_obs] real t_obs;   // Observed times
  array[n_fit] real t_fit;   // Times for predictions
  real t0;                   // Initial time

  // Priors for random effects:
  vector[K] mu_s1;           // Mean of sources for isotope 1
  vector[K] mu_s2;           // Mean of sources for isotope 2
  vector<lower=0>[K] sigma_s1; // SD of sources for isotope 1
  vector<lower=0>[K] sigma_s2; // SD of sources for isotope 2
...
model {
  ...
  for (i in 1:n_obs) {
    mu_arr[i] = ode_rk45(bdmm, (i == 1 ? y0 : mu_arr[i - 1]), 
                         (i == 1 ? t0 : t_obs[i - 1]), 
                         {t_obs[i]}, lambda, K, 
                         to_vector(s1[i, ]), to_vector(s2[i, ]), q1, q2, D, beta, t_mean, t_sd)[1];
...
generated quantities {
  ...
  // For predictions use mean source values (mu_s1 and mu_s2):
  vector[K] s1_mean = mu_s1;
  vector[K] s2_mean = mu_s2;

  y_fit = ode_rk45(bdmm, y0, t0, t_fit,
                   lambda, K, s1_mean, s2_mean, q1, q2,
                   D, beta, t_mean, t_sd);
