functions {
  ...
  //lambda over time function 
  real lambda_over_time(vector t, vector y, real t_est){
    int N = num_elements(t);
    int i = 1;

    // Find interval [t[i], t[i+1]] such that t[i] <= t_est < t[i+1]
    for (n in 1:(N - 1)) {
      if (t[n] <= t_est && t_est <= t[n + 1]) {
        i = n;
        break;
      }
    }
...
//attention! here t_S1 in lambda is because t_S1=t_obs in Marin et al.
  vector bdmm(real time,
    ...
    vector[2] derivative;
    derivative[1] = lambda_over_time(t_S1,lambda,time) * (-y[1] + Mix(K, time, s1,t_S1, q1, D, beta, t_mean, t_sd));
    derivative[2] = lambda_over_time(t_S2,lambda,time) * (-y[2] + Mix(K, time, s2,t_S2, q2, D, beta, t_mean, t_sd));
    return derivative;
  }
}
...
data {
  ...
  //mean_lambda for the fit
  vector[n_obs] lambda_mean; 
  
  // Hyperparameters for lambda prior (Gamma shape–rate)
  array[n_obs] real<lower=0> alpha_lambda;
  array[n_obs] real<lower=0> beta_lambda;
...
parameters {
  ...
  vector<lower=0>[n_obs] lambda;              // Turnover rate
...
model {
  ...
  for (n in 1:n_obs){
    lambda[n] ~ gamma(alpha_lambda[n], beta_lambda[n]);
  }
...
generated quantities {
  ...
  y_fit = ode_rk45(bdmm, y0, t0, t_fit,
                   lambda_mean, K, s1_mean, t_obs, s2_mean,t_obs, q1, q2,
                   D, beta, t_mean, t_sd);
