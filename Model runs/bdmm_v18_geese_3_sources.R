...
setwd("/home/smw-log/Bureau/Emilie_BDMM/")
...
# 
# prior<- rgamma(n=1000,shape=alpha_lambda,rate=beta_lambda)
# dens_prior<- density(prior)
# plot(dens_prior, xlab="",ylab="",main="",xlim=c(0,1)
#      ,lwd=3, cex.lab=1.3,las=1)
...
write.csv(bdmm_fitted$summary(), file = "v_18_geese_dernier_fitted.csv", row.names = FALSE)
...
write.csv(bdmm_generated_summary, file = "v18_geese_dernier_summary.csv", row.names = FALSE)
...
write.csv(lambda_fitted, file = "geese_lambda_fitted.csv", row.names = FALSE)
...
for (i in 1:ncol(lambda_fitted)){
  ...
  #prior
  prior= rgamma(n=nrow(lambda_fitted),shape=alpha_lambda,rate=beta_lambda)
  dens_prior<-density(prior)
  
  y_max=max(dens_prior[[2]],dens_post[[2]])
  title_graph="lambda_geese.tiff"
  tiff(title_graph,  width =4 , height = 5, units = "in", res = 300)
  plot(dens_prior,col=cols[1], xlab="",ylab="",main="",xlim=c(0,0.12)
...
tiff("fit_geese_C_v18.tiff",  width =4 , height = 5, units = "in", res = 300)
...
tiff("fit_geese_N_v18.tiff",  width =4 , height = 5, units = "in", res = 300)
