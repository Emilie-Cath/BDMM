...
bdmm <- cmdstan_model("bdmm_v17.stan", force_recompile = TRUE)
...
write.csv(bdmm_fitted$summary(), file = "oysters_fitted_v17.csv", row.names = FALSE)
...
write.csv(bdmm_generated_summary, file = "oysters_generated_v17.csv", row.names = FALSE)
...
#carbon
dev.off() 
tiff("fit_BDMM_C_oysters_v17.tiff", , width =4 , height = 5, units = "in", res = 300)
...
dev.off()
#nitrogen
tiff("fit_BDMM_N_oysters_v17.tiff", , width =4 , height = 5, units = "in", res = 300)
...
tiff("Contrib_BDMM_oysters_v17.tiff", , width =4 , height = 5, units = "in", res = 300)
